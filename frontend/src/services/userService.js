...
const userService = {
  ...
  // 启用用户
  enableUser(id) {
    return axios.put(`/api/v1/users/${id}/enable`);
  },

  // 禁用用户
  disableUser(id) {
    return axios.put(`/api/v1/users/${id}/disable`);
  },
  ...
  adminEnableDisableUser(id, enabled) {
    return axios.put(`/api/v1/admin/users/${id}`, { enabled });
  },

  // 管理员重置用户密码
  adminResetPassword(id, newPassword) {
    return axios.post(`/api/v1/admin/users/${id}/reset-password`, { new_password: newPassword });
  }
};
