下面是**基于我们截至目前全部共识**，重新整理、结构化、冻结的一份 **完整 PRD（含 DB 设计）**。
这是一份**可以直接交付给研发/评审/立项的版本**，逻辑已经“收敛”，不会再出现中途推翻的点。

> **PRD v1.0（重构冻结版）**
> 范围：一期实现 + 二期展望
> 关键词：**无中间表、规则驱动、列级业务规则、模板映射、AI 能力配置**

---

# 日本清关 / 派送 Excel 自动处理系统

**PRD v1.0（一期冻结版）**

---

## 1. 背景与目标

### 1.1 背景

日本清关与派送业务中，Excel 文件处理存在以下问题：

* 原始 Excel 来源多、模板不统一
* 字段规则复杂，人工修复成本高
* 部分字段需计算，部分字段需 AI 语义补齐
* 输出 Excel 对字体、样式、合并单元格有严格要求
* 处理过程缺乏结构化规则与可追溯性

---

### 1.2 产品目标

构建一个 Web 系统，实现：

1. 用户上传 **原始 Excel 文件**
2. 系统基于 **规则配置**：

   * 映射原始字段到最终字段
   * 执行字段级修复 / 计算 / AI 生成
3. 生成 **最终合规 Excel**
4. 输出 **原始文件 / 结果文件 / 对比文件**
5. 提供 **任务历史与操作日志**

---

## 2. 用户角色与权限（一期）

### 2.1 用户角色

| 角色  | 能力        |
| --- | --------- |
| 管理员 | 用户管理、规则配置 |
| 操作员 | 上传文件、执行处理 |

---

### 2.2 权限策略（一期冻结）

* 系统必须登录使用
* **不做用户级数据隔离**
* **不做下载鉴权**
* 管理员权限仅体现在配置功能入口

---

## 3. 文件类型与输入规则（一期）

### 3.1 支持的文件类型

| 文件类型 | 标识         |
| ---- | ---------- |
| 清关文件 | `customs`  |
| 派送文件 | `delivery` |

---

### 3.2 上传规则

* 一次仅支持 **1 个 Excel 文件**
* 文件格式：`.xlsx`
* 支持多 Sheet：

  * 仅 1 个 Sheet 参与处理
  * 其余 Sheet 原样复制
* 支持合并单元格

---

### 3.3 业务输入字段

#### 清关文件（customs）

* 航空号（flight_no）
* 报关日期（declare_date）
* 唯一编码（unique_code）

#### 派送文件（delivery）

* 唯一编码（unique_code）

---

## 4. 系统处理流程（一期）

```text
原始 Excel
   │
   │ ① 模板映射（template_mappings）
   │    - 识别 Sheet
   │    - 原始列 → source_key
   │
   │ ② 字段映射（MAP 规则）
   │    - 原始 → 最终字段来源
   │
   │ ③ 字段处理（PROCESS 规则）
   │    - RULE_FIX / CALC / AI
   │
   │ ④ Excel 样式应用（ExcelConfig）
   │
   ▼
最终 Excel + 对比文件 + 统计
```

> 任一步失败 → **任务整体失败（阻断）**

---

## 5. 规则体系（核心设计）

系统采用 **规则驱动架构**，规则分为三类配置，但职责清晰、不混用。

---

## 5.1 模板映射规则（Template Mapping）

### 目的

解决：

* 原始 Excel 列顺序变化
* 列标题不一致
* 多来源模板差异

### 能力

* 通过 **Sheet 名 / 索引**识别目标 Sheet
* 通过 **表头别名 / 位置兜底** 绑定原始列
* 将原始列绑定为系统内部 `source_key`

---

## 5.2 字段映射规则（MAP）

### 目的

定义 **最终 Excel 的每一列，其值从哪里来**

### 操作类型

| 操作     | 含义        |
| ------ | --------- |
| COPY   | 从原始列直接复制  |
| DROP   | 最终文件不保留该列 |
| DERIVE | 从多个字段生成   |

---

## 5.3 字段处理规则（PROCESS）

字段级业务规则，**按列定义**。

### 字段类型（必须标注）

| 类型       | 含义           | 依赖     |
| -------- | ------------ | ------ |
| RULE_FIX | 规则校验并修正      | 仅自身    |
| CALC     | 计算字段         | 依赖其他字段 |
| AI       | AI 语义生成 / 修正 | 依赖多个字段 |

### 执行顺序（一期写死）

1. RULE_FIX
2. CALC
3. AI

---

## 5.4 AI 能力配置（列级）

AI 相关能力 **不写死在规则里**，统一配置：

* 使用哪种能力
* prompt 模板
* 依赖字段
* 输出约束
* 失败策略

---

## 6. Excel 输出规范（一期冻结）

### 6.1 字体

* 默认字体：**可配置**
* 所有写入/修改的单元格必须使用默认字体

### 6.2 日期与数值

* 日期统一：`YYYY-MM-DD`
* 不出现科学计数法
* 文本保留前导零

### 6.3 合并单元格

* 合并范围由配置指定
* 不自动推断

### 6.4 样式

* 样式为 **模板级固定表现**
* 可按列/行/范围配置（如：B 列标黄）

---

## 7. 输出物（一期）

| 输出       | 必须 |
| -------- | -- |
| 原始 Excel | ✅  |
| 最终 Excel | ✅  |
| 对比 Excel | ✅  |

### 对比内容

* Sheet
* 行 / 列
* 原值 / 新值
* 变化类型

---

## 8. 任务与日志

### 8.1 任务

* 一个任务 = 一个 Excel
* 状态：processing / success / failed
* 记录参数、文件路径、统计、错误信息

### 8.2 操作日志

* 登录成功 / 失败
* 创建任务
* 任务成功 / 失败
* （可选）文件下载

---

# 9. 数据库设计（一期冻结）

---

## 9.1 users

| 字段                      |
| ----------------------- |
| id                      |
| username (unique)       |
| password_hash           |
| display_name            |
| role (admin/operator)   |
| enabled                 |
| last_login_at           |
| created_at / updated_at |

---

## 9.2 tasks

| 字段                                             |
| ---------------------------------------------- |
| id                                             |
| created_by_user_id                             |
| file_type                                      |
| unique_code                                    |
| flight_no                                      |
| declare_date                                   |
| status                                         |
| error_code / error_message / error_detail_json |
| stats_json                                     |
| started_at / finished_at                       |
| created_at / updated_at                        |

---

## 9.3 task_files

| 字段                               |
| -------------------------------- |
| id                               |
| task_id                          |
| original_file_name / storage_key |
| result_file_name / storage_key   |
| diff_file_name / storage_key     |
| mime_type                        |
| size_bytes                       |
| created_at / updated_at          |

---
| 字段                 | 含义                             |
| ------------------ | ------------------------------ |
| template_code      | 模板唯一标识，用于规则绑定                  |
| file_type          | 文件业务类型（delivery / customs）     |
| source_sheet       | 源 Excel 中需要处理的 Sheet           |
| target_sheet       | 生成 Excel 的 Sheet               |
| header_row         | 表头所在行                          |
| data_start_row     | 数据起始行                          |
| match_strategy     | 列匹配策略（header_first / col_only） |
| target_schema_json | 目标文件列结构定义                      |
| enabled            | 是否启用                           |
| remark             | 备注说明                           |

二、rule_tables（规则集定义）
2.1 DB 表结构
CREATE TABLE rule_tables (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  rule_table_code VARCHAR(100) NOT NULL UNIQUE,
  rule_table_name VARCHAR(255) NOT NULL,
  file_type VARCHAR(50) NOT NULL,
  stage VARCHAR(20) NOT NULL,
  enabled TINYINT DEFAULT 1,
  description VARCHAR(255),
  created_at DATETIME,
  updated_at DATETIME
);


| 字段              | 含义            |
| --------------- | ------------- |
| rule_table_code | 规则集唯一编码       |
| rule_table_name | 规则集名称         |
| file_type       | 文件类型          |
| stage           | MAP / PROCESS |
| enabled         | 是否启用          |
| description     | 规则集说明         |

三、rule_items（⭐ 核心规则配置）

这一张表同时承载 MAP 和 PROCESS
用 rule_table_code + stage 区分

3.1 DB 表结构（最终推荐）
CREATE TABLE rule_items (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  rule_table_code VARCHAR(100) NOT NULL,
  enabled TINYINT DEFAULT 1,
  order_no INT NOT NULL,

  target_col CHAR(2) NOT NULL,
  target_header VARCHAR(255),

  -- MAP 阶段
  map_op VARCHAR(50),
  source_col CHAR(2),
  source_header VARCHAR(255),
  derive_type VARCHAR(50),
  derive_rule JSON,

  -- PROCESS 阶段
  field_type VARCHAR(50),
  executor_type VARCHAR(20),
  depends_on VARCHAR(100),
  proc_rules JSON,
  on_fail VARCHAR(20),

  remark VARCHAR(255),
  created_at DATETIME,
  updated_at DATETIME
);

3.2 字段含义（非常重要）
通用字段
字段	含义
rule_table_code	归属哪个规则集
enabled	是否启用
order_no	执行顺序
target_col	目标 Excel 列字母
target_header	目标表头
MAP 相关字段（仅 MAP 规则用）
字段	含义
map_op	COPY / DERIVE
source_col	源列字母
source_header	源表头
derive_type	CONST / INPUT_PARAM / EXPR
derive_rule	派生规则（JSON）
PROCESS 相关字段（仅 PROCESS 规则用）
字段	含义
field_type	COPY / FORMAT / DEFAULT / CALC / RULE_FIX / AI
executor_type	program / ai
depends_on	依赖的列（如 C,D）
proc_rules	处理规则（JSON）
on_fail	block / warn


四、ai_field_capabilities（AI 字段能力配置）
4.1 DB 表结构
CREATE TABLE ai_field_capabilities (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  file_type VARCHAR(50) NOT NULL,
  target_col CHAR(2) NOT NULL,
  model VARCHAR(50),
  prompt_template TEXT,
  depends_on VARCHAR(100),
  enabled TINYINT DEFAULT 1,
  remark VARCHAR(255),
  created_at DATETIME,
  updated_at DATETIME
);

4.2 字段含义
字段	含义
file_type	文件类型
target_col	目标列
model	使用的模型
prompt_template	Prompt 模板
depends_on	依赖字段
enabled	是否启用
remark	说明




## 9.8 operation_logs

| 字段                      |
| ----------------------- |
| id                      |
| user_id                 |
| action                  |
| entity_type / entity_id |
| success                 |
| message                 |
| detail_json             |
| created_at              |

---

## 10. 二期规划（不影响一期）

* 批量文件上传
* 文件级成功/失败
* 变化高亮（diff 高亮）
* Web 端差异预览
* 规则版本管理
* 下载鉴权 / 脱敏

---

## 11. 当前状态

* ✅ PRD 已冻结
* ✅ DB 结构已确定
* ✅ 规则模型已统一
* 🚀 可直接进入：

  * 接口设计
  * 后端实现
  * 规则填表

---

如果你愿意，**下一步最自然的是**：

👉 **基于这份 PRD，直接生成 FastAPI 接口设计文档（API Spec）**
👉 或 **把 PDF 的 3.4 全量转成 rule_items 实例表**

你只需说下一步要哪一个。
