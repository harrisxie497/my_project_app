...
def query_calc_copy_format_columns():
    """查询CALC、COPY、FORMAT类型的列"""
    print("=" * 100)
    print("查询CALC、COPY、FORMAT类型的列")
    print("=" * 100)
    ...
    try:
        ...
        WHERE file_type = 'CUSTOMS' AND map_op IN ('CALC', 'COPY', 'FORMAT')
        ORDER BY map_op, target_col
        """
        
        cursor.execute(sql)
        results = cursor.fetchall()
        
        print(f"\n找到 {len(results)} 个CALC、COPY、FORMAT类型的列:\n")
...
if __name__ == "__main__":
    query_calc_copy_format_columns()
