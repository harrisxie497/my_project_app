...
def execute_ai_field(ai_service, field_config, rule_config, test_data, processed_data):
    ...
    输入：
        ...
        - rule_config: 规则配置
        - test_data: 原始测试数据
        - processed_data: 已处理的数据（包含依赖字段）
    """
    system_prompt = rule_config.get('system_prompt')

    # 特殊处理F列（重量）- 按照系统提示词的格式构建输入
    if field_config['target_col'] == 'F':
        ...
        for i, row_data in enumerate(test_data):
            # 从已处理数据中获取H列和I列的值
            h_value = processed_data[i].get('H') if 'H' in processed_data[i] else row_data.get('H', '')
            i_value = processed_data[i].get('I') if 'I' in processed_data[i] else row_data.get('I', '')
