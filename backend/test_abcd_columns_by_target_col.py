"""
测试A、B、C、D四列的处理结果 - 使用target_col查找
"""
...
def test_abcd_columns_by_target_col():
    """测试A、B、C、D四列的处理结果 - 使用target_col查找"""
    print("=" * 100)
    print("测试A、B、C、D四列的处理结果 - 使用target_col查找")
    print("=" * 100)
    ...
    try:
        ...
        # 只显示A、B、C、D列的处理结果（使用target_col查找）
        print("\n" + "-" * 100)
        print("处理后的列数据（A、B、C、D列）- 使用target_col查找")
        print("-" * 100)
        
        for col in processed_column_data:
            if col.get('target_col') in ['A', 'B', 'C', 'D']:
...
if __name__ == "__main__":
    test_abcd_columns_by_target_col()
