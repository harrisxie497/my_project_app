"""
检查processed_column_data的顺序是否与用户提到的表头顺序一致
"""

import re

def check_processed_column_order():
    """检查processed_column_data的顺序"""
    print("=" * 100)
    print("检查processed_column_data的顺序")
    print("=" * 100)
    ...
    # 查找最近的"处理列配置"日志
    print(f"\n\n最近的处理列配置日志（CUSTOMS任务）:")
    count = 0
    for line in lines:
        if "处理列配置 - 列名:" in line and "customs_processor" in line and count < 35:
            print(f"  {line.strip()}")
            count += 1
    ...
    for line in lines:
        if "处理列配置 - 列名:" in line and "customs_processor" in line:
            ...
            if match:
                ...
                # 提取表头
                match2 = re.search(r"表头: ([^,]+),", line)
                if match2:
                    header = match2.group(1)
                    processed_headers.append(header)
    
    print(f"\n处理顺序（共{len(processed_headers)}列）:")
    ...
    if processed_headers == user_headers:
        ...
        # 找出不一致的地方
        print(f"\n  不一致的列:")
        for idx, (processed_header, user_header) in enumerate(zip(processed_headers, user_headers), start=1):
            if processed_header != user_header:
                print(f"    第{idx}列: 处理顺序='{processed_header}', 用户提到的='{user_header}'")
