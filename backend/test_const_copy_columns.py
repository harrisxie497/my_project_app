...
def test_const_copy_columns():
    """测试map_op为CONST和COPY的列"""
    print("=" * 100)
    print("测试map_op为CONST和COPY的列")
    print("=" * 100)
    ...
    try:
        ...
        # 只处理map_op为CONST和COPY的列
        const_copy_pipelines = [p for p in field_pipelines if p.get('map_op') in ['CONST', 'COPY']]
        
        print(f"\nmap_op为CONST和COPY的列配置:")
        for pipeline in const_copy_pipelines:
        ...
        # 只显示map_op为CONST和COPY的列的处理结果
        print("\n" + "-" * 100)
        print("处理后的列数据（map_op为CONST和COPY的列）")
        print("-" * 100)
        
        for col in processed_column_data:
            if col.get('target_col') in [p.get('target_col') for p in const_copy_pipelines]:
