...
# 从 update_system_prompt.sql 中提取的系统提示词
SYSTEM_PROMPTS = {
...
def restore_system_prompt(rule_ref: str, system_prompt: str) -> bool:
    """
    恢复数据库中指定规则的系统提示词
    ...
    try:
        ...
        print(f"✓ 恢复规则 {rule_ref} 成功，影响行数: {affected_rows}")
        return affected_rows > 0
        
    except Exception as e:
        print(f"✗ 恢复规则 {rule_ref} 失败: {str(e)}")
        return False
...
def main():
    ...
    for rule_ref, system_prompt in SYSTEM_PROMPTS.items():
        ...
        if restore_system_prompt(rule_ref, system_prompt):
            success_count += 1
        else:
            fail_count += 1
        print()
