...
def test_efghi_detailed():
    """测试查看E-I列的详细配置和源数据"""
    print("=" * 100)
    print("测试查看E-I列的详细配置和源数据")
    print("=" * 100)
    ...
    for col in result['column_data']:
        if col.get('head') in ['貨物個数', '貨物重量', '重量単位コード', '品名', '材质']:
            print(f"\n列 {col.get('head')}:")
    ...
    try:
        ...
        # 只处理E、F、G、H、I列
        efghi_pipelines = [p for p in field_pipelines if p.get('target_col') in ['E', 'F', 'G', 'H', 'I']]
        
        print(f"\nE、F、G、H、I列的配置:")
        for pipeline in efghi_pipelines:
        ...
        # 只显示E、F、G、H、I列的处理结果
        print("\n" + "-" * 100)
        print("处理后的列数据（E、F、G、H、I列）")
        print("-" * 100)
        
        for col in processed_column_data:
            if col.get('target_col') in ['E', 'F', 'G', 'H', 'I']:
